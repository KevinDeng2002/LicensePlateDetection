...
from dataset.LPRDataset import CHARS, CHARS_DICT, LPRDataLoader
...
def get_parser():
    ...
    parser.add_argument('--test_img_dirs', default="./data/test2", help='the test images path')
    ...
    parser.add_argument('--pretrained_model', default='./weights/lprnet.weights', help='pretrained base model')
...
def Greedy_Decode_Eval(Net, datasets, args):
    ...
    batch_iterator = iter(data.DataLoader(datasets, args.test_batch_size, shuffle=True, num_workers=args.num_workers,
...
def Greedy_Decode_Eval_1image(net, image):
    # forward
    preb = net(image)
    # greedy decode
    ...
    for i, label in enumerate(preb_labels):
        for tww in range(0, len(np.asarray(label))):
            result += (CHARS[np.asarray(label)[tww]])
    return result
